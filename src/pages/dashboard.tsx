...
import { PlayerListPage as ShowCaseList } from 'pages/players';
...
function DashboardPage() {
  ...
  const playerStat = useSWR<PaginatedInterface<PlayerInterface>>(
    () => `/players`,
    () => getPlayers({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Players',
      result: playerStat?.data?.totalCount,
      isLoading: playerStat.isLoading,
    },
    {
      title: 'Referees',
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`players`} passHref legacyBehavior>
                  <Button
